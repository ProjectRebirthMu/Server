...
{
	return (SERVER_RANGE(this->m_index) != 0 && this->m_state != SERVER_OFFLINE && this->m_socket != INVALID_SOCKET);
}

bool CServerManager::CheckAlloc() // OK
{
	return (this->m_IoRecvContext != 0 && this->m_IoSendContext != 0);
}

void CServerManager::AddServer(int index, const char* ip, SOCKET socket) // OK
{
	this->m_index = index;
	...
	this->m_IoRecvContext->wsabuf.buf = reinterpret_cast<char*>(this->m_IoRecvContext->IoMainBuffer.buff);
	...
	this->m_IoSendContext->wsabuf.buf = reinterpret_cast<char*>(this->m_IoSendContext->IoMainBuffer.buff);
...
void CServerManager::SetServerInfo(const char* name, WORD port, WORD code) // OK
{
