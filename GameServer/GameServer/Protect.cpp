...
bool CProtect::AuthServerOnlineSend() // OK
{
	//#if(PROTECT_STATE==1)
	...
	SDHP_AUTH_SERVER_ONLINE_SEND pMsg;
	...
	for(int n=4;n < sizeof(SDHP_AUTH_SERVER_ONLINE_SEND);n++)
	{
		((BYTE*)&pMsg)[n] ^= pMsg.EncKey+0x1A;
		((BYTE*)&pMsg)[n] -= 0x69;
	}

	if(send(this->m_socket2,(char*)&pMsg,pMsg.size,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSAEWOULDBLOCK)
		{
			return 0;
...
void CProtect::OnlineAuth() // OK
{
	...
	if(this->m_socket2 != INVALID_SOCKET)
	{
		return;
	}

	this->m_socket2 = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);

	if(this->m_socket2 == INVALID_SOCKET)
	{
		closesocket(this->m_socket2);
		this->m_socket2 = INVALID_SOCKET;
		return;
	}
	...
	target.sin_family = AF_INET;
	target.sin_port = htons(12556);
	target.sin_addr.s_addr = inet_addr(this->GetEncryptedString(gProtectString3,sizeof(gProtectString3)));
	
	if(target.sin_addr.s_addr == INADDR_NONE)
	{
		HOSTENT* host = gethostbyname(this->GetEncryptedString(gProtectString3,sizeof(gProtectString3)));
	...
	if(connect(this->m_socket2,(SOCKADDR*)&target,sizeof(target)) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSAEWOULDBLOCK)
		{
			LogAdd(LOG_RED,"[AuthServer] Error Connection (%d)",AuthCount);
			closesocket(this->m_socket2);
			this->m_socket2 = INVALID_SOCKET;
			return;
		}
	...
	if(this->AuthServerOnlineSend() == 0)
	{
		closesocket(this->m_socket2);
		this->m_socket2 = INVALID_SOCKET;
		return;
	...
	if(recv(this->m_socket2,(char*)&this->m_AuthOnlineInfo,sizeof(this->m_AuthOnlineInfo),0) != sizeof(SDHP_AUTH_SERVER_ONLINE_RECV))
	{
		closesocket(this->m_socket2);
		this->m_socket2 = INVALID_SOCKET;
		return;
	...
	if(this->m_AuthOnlineInfo.EncKey != this->m_EncKey)
	{
		closesocket(this->m_socket2);
		this->m_socket2 = INVALID_SOCKET;
		return;
	...
	for(int n=4;n < sizeof(SDHP_AUTH_SERVER_ONLINE_RECV);n++)
	{
		((BYTE*)&this->m_AuthOnlineInfo)[n] += 0xDA;
		((BYTE*)&this->m_AuthOnlineInfo)[n] ^= this->m_AuthOnlineInfo.EncKey+0x25;
	}

	if(this->m_AuthOnlineInfo.result == 0x01)
	{
		AuthCount = 0;
	}
	else
	{
		LogAdd(LOG_RED,"[AuthServer] Error License (%d)",AuthCount);
	}
